...
class TableoutputdocumentolocalPipelineLoader:
    """
    Loader para: Table output documento local
    Tipo: TableOutput
    """
    ...
    def load(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        logger.info(f"🔄 Iniciando carga: Table output documento local")
        ...
        try:
            ...
            return {
                ...
                "target": "Table output documento local"
            }
            
        except Exception as e:
            logger.error(f"❌ Erro na carga Table output documento local: {e}")
            raise
