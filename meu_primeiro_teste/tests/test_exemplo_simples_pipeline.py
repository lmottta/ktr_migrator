"""
Testes para pipeline exemplo_simples
"""
import pytest
import pandas as pd
from unittest.mock import Mock, patch

from src.pipelines.exemplo_simples_pipeline import ExemploSimplesPipeline


class TestExemploSimplesPipeline:
    """Testes do pipeline exemplo_simples"""

    def setup_method(self):
        """Setup para cada teste"""
        self.config = {"log_level": "DEBUG", "environment": "test"}
        self.pipeline = ExemploSimplesPipeline(self.config)
    ...
    def test_run_pipeline_success(self):
        ...
        with patch.object(self.pipeline, "extract_data") as mock_extract, patch.object(
            self.pipeline, "transform_data"
        ) as mock_transform, patch.object(self.pipeline, "load_data") as mock_load:
            # Setup mocks
